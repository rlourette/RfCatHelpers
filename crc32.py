# // ****************************************************************************
# /// Calculates the 32bit CRC on the given byte stream using the Ethernet CRC
# ///
# /// 	f(x) = x^32 + x^26 + x^23 + x^22 + x^16 + x^12 + x^11 + x^10 + x^8
# ///			+ x^7 + x^5 + x^4 + x^2 + x + 1
# ///
...
# /// algorithm (typically 0xFFFFFFFF) be pre-inverted to 0x00000000.
# ///
...
#  */
# 	_s_CrcTable = [ 0x00000000, 0x77073096, 0xEE0E612C,
...
# //
	def CalculateCrc32(buf:bytes, crc: int):
		assert isinstance(buf, bytes)
		assert crc >= 0
		assert len(buf)

#ifdef DYNAMIC_CRC_TABLE
		if None is CRC32.s_CrcTable:
			CRC32.MakeCrcTable()
#endif

		crc = crc ^ 0xffffffff
		foo = CRC32.s_CrcTable
		# // no need to optimize
		for i in range(len(buf)):
			c = buf[i]
			cc = (crc ^ c) & 0xff
			entry = CRC32.s_CrcTable[cc]
			crc = CRC32.s_CrcTable[(crc ^ c) & 0xff] ^ (crc >> 8)
		crc = crc ^ 0xffffffff
	...
	def test():
		...
		CRC32.MakeCrcTable()
		for n in range(256):
			assert CRC32.s_CrcTable[n] == _s_CrcTable[n]
		crc = CRC32.CalculateCrc32(testData, 0)
