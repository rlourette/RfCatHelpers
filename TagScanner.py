...
parser.add_argument('-fa', action="store", default="433272000", dest="startFreq",help='Frequency to start scan at, defaults to 433000000',type=int)
parser.add_argument('-fb', action="store", default="433272000", dest="endFreq",help='Frequency to end scan at, defaults to 434000000',type=int)
parser.add_argument('-fs', action="store", default="50000", dest="stepFreq",help='Frequency step for scanning, defaults to 50000',type=int)
parser.add_argument('-ft', action="store", default="1000", dest="timeStepFreq",help='Frequency step delay, defaults to 1000 milliseconds',type=int)
...
print(
	"Scanning for AM/OOK Remotes... Press " + BOLD + WHITE + "<enter>" + ENDC + " to stop and " + BOLD + WHITE + " any key" + ENDC + " to continue\n")
...
if(results.replayKey == True and len(sortedKeys) > 0):
	...
	if len(sorted_lens) > 0:
		...
		if(results.verbose == True):
			print("\nFound most keys in string have a length of " + str(
				searchLen) + " using those keys to calculate common key.")
		foundKeys = []
		for a in allstrings:
			if(len(a) == searchLen):
